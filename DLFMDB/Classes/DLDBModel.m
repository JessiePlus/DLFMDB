...
#import "DLDBModel.h"
#import "DLDBTool.h"
@implementation DLDBModel
+ (void)initialize
{
    if (self != [DLDBModel class]) {
        [self createTable];
    }
}
...
{
    __block BOOL res = NO;
    DLDBTool *DLDB = [DLDBTool shareInstance];
    [DLDB.dbQueue inDatabase:^(FMDatabase *db) {
...
+ (BOOL)createTable
{
    ...
    DLDBTool *DLDB = [DLDBTool shareInstance];
    [DLDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        NSString *tableName = NSStringFromClass(self.class);
...
{
    DLDBSQLState *query = [[DLDBSQLState alloc] object:self.class type:WHERE key:columnName opt:@"=" value:columnValue];
    
    id record = [self.class findFirstByCriteria:[query sqlOptionStr]];
    if (record) {
...
- (BOOL)save
{
    ...
    for (int i = 0; i < self.columeNames.count; i++) {
        ...
        if ([proname isEqualToString:[self.class getPKName][@"pkColumn"]]) {
            continue;
        }
...
{
    //判断是否是JKBaseModel的子类
    for (DLDBModel *model in array) {
        if (![model isKindOfClass:[DLDBModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [DLDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            DLDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
...
+ (NSMutableArray *)PKAndColumnModify{
    ...
    for (int i = 0; i < properties.count ; i++) {
        ...
        DLDBColumnDes *des = desDic[property];
        if (desDic[property]) {
...
/** 如果子类中有一些property不需要创建数据库字段,或者对字段加修饰属性   具体请参考DLDBColumnDes类*/
