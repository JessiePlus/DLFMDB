...
static DLDBTool *_instance = nil;
@implementation DLDBTool
...
+ (NSString *)dbPathWithDirectoryName:(NSString *)directoryName
{
    ...
    if (directoryName == nil || directoryName.length == 0) {
        docsdir = [docsdir stringByAppendingPathComponent:@"DLDB"];
    } else {
...
- (BOOL)changeDBWithDirectoryName:(NSString *)directoryName
{
    ...
    _instance.dbQueue = [[FMDatabaseQueue alloc] initWithPath:[DLDBTool dbPathWithDirectoryName:directoryName]];
    ...
    if (numClasses > 0 )
    {
        ...
        for (int i = 0; i < numClasses; i++) {
            if (class_getSuperclass(classes[i]) == [DLDBModel class]){
                id class = classes[i];
